...
| **Fold set v5 (replication cohort)** | `fold_sets/binders_v5.csv`, `decoys_v5.csv`, `fold_set_v5.csv`; ESMFold2 arm in `pae_esmfold2_v5.csv` |
...
| Regenerated fold sets, folded on all five arms | Fold set v5 is built and four arms are folded — AF3, AF2 and ESMFold2 under held conditions (rows 32-33), and Boltz-2.1 under changed ones (row 33c). The fine-tuned arm was not folded; its alignment file was lost with a cleared scratch filesystem and restoring it was judged not worth the remaining time. The primary RQ1 table remains v4, chiefly because the sequence model was never scored on v5 at all. |
...
| 10 | `pae_anchors_ic` is the best structural feature in **nine of ten** measurements once between-allele scale is removed. The exception is AF3 on fold set v2, where z-scored it gives 0.855 against `pae_anchor2`'s 0.875, driven by one allele: for HLA-A\*02:01 the P2 PAE alone gives 0.979 while the C-terminal PAE gives 0.583, so averaging them halves the signal to 0.688. Note `pae_anchors` and `pae_anchors_ic` are identical there (both [1,−1]), so this is not the IC derivation but the assumption that both anchors contribute; on 12 binders and 12 decoys it may be noise, and a per-position rather than averaged feature is the obvious follow-up | v2 and v4 | `auroc_af2_v*.csv`, `auroc_esmfold2_v4.csv`, `auroc_boltz_v4.csv`, `auroc_af3_v4.csv`, `pae_af3_v4_z.csv`, `pae_esmfold2_v5.csv`, `pae_af2_v5.csv`, `pae_af3_v5.csv` | verified, extended 8 Sep |
...
| 16 | P5 basic-residue chelation requires a configuration of pocket residues, not Asp9 alone. Measured in wild-type B\*08:01 (PDB 4QRT, peptide ELNRKMIYM), the Lys5 ammonium makes three polar contacts: **Asp9 2.78 Å, Ser97 2.81 Å, Asp74 2.86 Å**, all NZ to a side-chain oxygen, with Tyr116 at 3.03 Å (CE–OH) packing against the side chain. **Thr69 is not a contact** — 5.74 Å in 4QRT, 6.20 Å in 8ESH — and is removed from the configuration. Asn70, named in the crystallographic report (ref. 51), is a packing contact rather than a polar one: CB–OD1 3.49 Å in 4QRT, CG–OD1 3.24 Å in 8ESH, i.e. against the aliphatic chain, not the ammonium. The chimera reproduces the anchor through a partly different set: Ser97's hydroxyl is rotated away (3.97 Å, and to CB) and position 116 is Asn, not Tyr, which is why ref. 51 names Asn70 as nearest polar neighbour. B\*08:01 is the only one of the twelve alleles with heavy chains in `fold_sets/af3v5_inputs/` carrying Asp9+Asp74+Ser97; re-run across all 123 alleles confirms it as the sole carrier there too (P5 IC 1.93 bits), unchanged from the superseded 9/69/74/97 set, so the "exactly one of 123" sentence in §4.4.2 can be restored. At least three distinct configurations achieve P5 anchoring: B\*08:01 (Asp9/Asp74/Ser97), B\*37:01 (His9/Tyr74/Arg97), B\*14:01–02 (Tyr9/Asp74/Trp97) | n/a | `results/p5_contacts_4qrt.csv`, `results/p5_contacts_8esh.csv`, `scripts/p5_contacts.py`, `results/p5_anchor_residues.csv`, `results/p5_configuration_123.csv`, `scripts/p5_configuration.py`; configuration originally proposed by Chris Thorpe, 21 Aug | verified 12 Sep |
...
| 18 | **No European-specific performance gap.** The pre-specified direct test — Europe-enrichment index against per-allele AUROC — is flat at **+0.013 (p 0.89)**, and European frequency controlling for global frequency gives +0.106 (p 0.24). What does hold is a *region-balanced* frequency effect in the opposite direction to the Europe-bias prediction: commoner alleles are predicted slightly worse, **−0.243 (p 0.007)** — mechanism supplied externally, see row 36. Note the measure means each of the seven regions counts equally regardless of how many populations AFND holds for it — deliberate, since sample-weighting would let European oversampling define "global", but it is region-balanced rather than global and is named so. The population-weighted alternative gives −0.186 (p 0.039), the two correlating at +0.878 | validation split, v3 model, 123 alleles | `results/afnd_direct_tests.csv`, `afnd_frequency_per_allele.csv` | verified 30 Aug |
...
| 22 | The four-architecture consensus (0.9047 vs AF3's 0.8575) is **variance reduction, not complementary information**: PC1 of the four z-scored scores alone gives 0.9033, a difference of 0.0014, with PC1 explaining 61.3% of variance. The gain does not track headroom (rho +0.203, p 0.60), so it is not the ceiling effect either. Diagnostic validated on synthetic data before use | fold set v4 | `results/consensus_diagnosis_v4.csv`, `consensus_diagnosis_v4_subsets.csv`, `consensus_diagnosis_v4_per_allele.csv` | verified 27 Aug |
...
| 32 | **Fold set v5 is a replication cohort, not a corrected v4.** Built from the regenerated table and clean split with the same nine alleles pinned (`--force-alleles`) and the same construction rules — PWM top decile for binders, anchor-matched below the 25th percentile for decoys. It shares **8 of 216 complexes with v4 (3.7%)**, because only 10.3% of the old validation split survives regeneration and the candidate pools differ accordingly (B\*08:01: 958 candidates against v4's 4,884). Any agreement between v4 and v5 is therefore replication across an almost-disjoint sample; any disagreement would confound the negative-sampling fix with sampling variation, so neither can be attributed to the fix alone | fold set v5, 216 complexes | `fold_sets/fold_set_v5.csv`, `binders_v5.csv`, `decoys_v5.csv` | verified 4 Sep |
| 33 | **The structural result replicates on the disjoint cohort, across three architectures held under fixed conditions.** Within-allele standardised `pae_anchors_ic`: AlphaFold 3 **0.8419** against 0.8575, AlphaFold 2 **0.8271** against 0.8423, ESMFold2 **0.8006** against 0.8053. The ordering is preserved exactly, every difference is under 0.02, and binder anchor-PAE is below decoy for all nine alleles on all three arms. Boltz-2.1 was also folded on v5 and gives 0.7093 against 0.7447, but under changed conditions and is not read as a replication — see row 33c. This addresses the negative-sampling concern more directly than regeneration alone: the finding survives not only cleaner data but an almost entirely different sample drawn under the same rules | fold set v5, 216 complexes each | `pae_af3_v5.csv`, `pae_af2_v5.csv`, `pae_esmfold2_v5.csv`, `pae_boltz_v5.csv`, `conf_af3_v5.csv` | verified 8 Sep |
| 33b | All three held-condition differences are **negative** and of similar size, which is not the shape of noise. The likely mechanism is candidate-pool size: regeneration leaves far fewer canonical-binder candidates per allele (958 for B\*08:01 against v4's 4,884), so the top decile is drawn from a smaller set and sits less far out on the allele's motif. v5 is therefore a slightly harder benchmark, by roughly 0.015 for the two AlphaFold arms. This does not affect the replication — ordering and magnitudes both survive — but it should be stated rather than left for a reader to infer | as above | `fold_sets/binders_v5.csv`; build log in `REPRODUCE.md`, 8 Sep | verified 8 Sep |
| 33c | **The Boltz arm cannot be compared across the two panels**, and the reason is not the fold set. Its client was upgraded from 0.43.0 to 0.50.0 between the runs, because the pinned version stopped completing runs against the live API once the launch credit lapsed (row 34), and whether the endpoint served identical weights a month later cannot be verified. Boltz also carries by far the weakest signal of any arm — binder-decoy anchor-PAE gaps of +0.014 to +0.510, against AlphaFold 3's +0.018 to +4.020 — so a larger between-sample swing is expected of it even with nothing else changed. The 0.035 drop is consistent both with ordinary variation and with a change in the served model, and the two cannot be separated. Reported as uninterpretable rather than as a negative result | fold set v5, 216 complexes | `pae_boltz_v5.csv` | verified 8 Sep |
| 34 | **The Boltz arm's method was undocumented until 8 September and depends on metered access.** `REPRODUCE.md` carried its results across eleven lines and its invocation in none; the driver lived in a `uv` project outside the repository and is now at `third_party/boltz_prediction/`. The v5 fold failed at first: predictions complete server-side but return no archive, because UCL's $100 launch-credit grant expired on 9 August and the v4 arm ran on that grant. The failure is identical on the pinned client (0.43.0) and the current one (0.50.0) and reports no HTTP status. With a new credit the arm ran, after deleting 45 local run directories whose recorded run IDs belonged to the previous workspace and were rejected by the new key. Two client behaviours found while diagnosing: 0.50.0 resolves its own output directory from the `name` argument and ignores the caller's path, and it deduplicates on entity sequences rather than names | n/a | `third_party/boltz_prediction/`; `REPRODUCE.md`, 8 Sep | verified 8 Sep |
| 35 | **Motif isolation as a determinant of per-allele accuracy is reproduced independently.** Tadros et al. (Genome Med 2025;17:25, the MixMHCpred3.0 paper) report a strong inverse correlation between an allele's binding-site distance to the nearest allele with known ligands and its leave-one-allele-out AUC, and propose 0.1 as the threshold beyond which predictions degrade. Their distance is BLOSUM62 over the 34 binding-site residues, not a divergence between PWMs, and they withhold whole alleles rather than peptides — so the agreement is not an artefact of a shared metric or a shared benchmark. Note the limit of the corroboration: binding-site distance is a *sequence* distance over groove residues, so it supports the determinant (distance from the characterised set) without supporting the motif-geometry framing specifically | external | Genome Med 2025;17:25, Fig. 3A | verified 12 Sep, from full text; **Fig. 3A itself not yet inspected** |
| 36 | **The region-balanced frequency effect is the isolation result on a frequency axis, not a separate finding.** Tadros et al. report the same sign — a slight negative correlation between allele frequency and LOA accuracy — and give the mechanism: frequency and isolation are not aligned, so a common allele can sit far from every characterised allele. The example they name is HLA-B\*08:01, which is also the allele this thesis finds hardest for the sequence model (rows 12, 16). Supersedes the "reasons unresolved" status of the −0.243 in row 18 | external + validation split | Genome Med 2025;17:25; `results/afnd_direct_tests.csv` | verified 12 Sep |
...
- ~~Thr69 predicts P5 anchoring.~~ It separates the nine alleles in
  ...
  deliberately omitted from the correspondence with Chris. Withdrawn a second time on
  independent grounds (12 Sep): Thr69 is 5.74 Å from the P5 side chain in wild-type
  B\*08:01 and 6.20 Å in the chimera, so it is not a contact at all
  (`results/p5_contacts_4qrt.csv`). The statistical and the geometric route reach the
...
- [x] **Re-run `p5_anchor_residues.csv` over positions 9/74/97** → **resolved 12 Sep.**
      `scripts/p5_configuration.py` recomputes the configuration over the measured
      contacts across all 123 alleles (`results/p5_configuration_123.csv`). B\*08:01
      remains the sole carrier (P5 IC 1.93 bits), unchanged from the superseded
      9/69/74/97 set — restore the "exactly one of 123" sentence in §4.4.2, see row 16.
...
- [x] Whether to rerun on regenerated data — **partly done 4 Sep.** Fold set v5 is
      built and the ESMFold2 arm folded and extracted. The result (row 33) replicates
      v4 to within 0.005 on a cohort sharing 3.7% of complexes, which is the point the
      rerun existed to establish. Folding the remaining four arms would extend the
      replication to the full architecture ordering but is not needed for the claim.
- [x] Whether to fold the remaining arms on v5 — **resolved 8 Sep.** AF3 and AF2 are
      done, giving a three-architecture replication under held conditions (row 33).
      Boltz was folded after a new API credit but under a changed client and possibly a
      changed model, so it is reported as uninterpretable (row 33c). The fine-tuned arm
      was not folded: its alignment file was lost with Beta's tmpfs — the third thing
      lost to that directory — and restoring it is a download rather than a difficulty,
      so this was a time judgement rather than an obstacle.

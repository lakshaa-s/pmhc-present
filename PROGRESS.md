...
| Regenerated fold sets, folded on all arms | Fold set v5 is built from the regenerated data and the ESMFold2 arm is complete (rows 32-33). AF3, AF2, Boltz and the fine-tuned arm are not folded on it, so the full five-architecture RQ1 table remains on v4. Roughly 12 h of GPU plus re-extraction; the ESMFold2 replication already establishes the load-bearing point. |
...
| 10 | `pae_anchors_ic` is the best structural feature in **seven of eight** measurements once between-allele scale is removed. The exception is AF3 on fold set v2, where z-scored it gives 0.855 against `pae_anchor2`'s 0.875, driven by one allele: for HLA-A\*02:01 the P2 PAE alone gives 0.979 while the C-terminal PAE gives 0.583, so averaging them halves the signal to 0.688. Note `pae_anchors` and `pae_anchors_ic` are identical there (both [1,−1]), so this is not the IC derivation but the assumption that both anchors contribute; on 12 binders and 12 decoys it may be noise, and a per-position rather than averaged feature is the obvious follow-up | v2 and v4 | `auroc_af2_v*.csv`, `auroc_esmfold2_v4.csv`, `auroc_boltz_v4.csv`, `auroc_af3_v4.csv`, `pae_af3_v4_z.csv`, `pae_esmfold2_v5.csv` | verified, extended 4 Sep |
...
| 33 | **The structural result replicates on the disjoint cohort.** ESMFold2's within-allele standardised `pae_anchors_ic` gives **0.8006 on v5 against 0.8053 on v4** — a difference of 0.005 across benchmarks sharing 3.7% of their complexes. Binder anchor-PAE is below decoy anchor-PAE for all nine alleles (gaps +0.232 to +2.127). This addresses the negative-sampling concern more directly than regeneration alone would have: the finding survives not just cleaner data but an almost entirely different sample drawn under the same rules | fold set v5, ESMFold2 arm | `pae_esmfold2_v5.csv` | verified 4 Sep |
...
- [ ] Whether to fold AF3, AF2, Boltz and the fine-tuned arm on v5 before submission.
      ~12 h GPU plus a day of re-extraction and re-analysis, against an already
      sufficient single-arm replication.
